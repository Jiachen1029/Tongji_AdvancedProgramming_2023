...
TString::TString(const TString& other) //拷贝构造函数
{
	if (other.content) 
	{
		len = other.len;
...
TString::~TString() //析构函数
{
	delete[] content;
}
char* TString::get_content()//获取content
{
	return content;
}
char* TString::get_content() const //获取content
{
	return content;
}
void TString::set(char* s) //设置content
{
	delete[] content;
	if (s) 
	{
