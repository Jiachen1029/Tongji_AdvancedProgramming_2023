...
public:
    Time(int, int, int);    // 声明构造函数
    friend void display(const Time&, const Date&); // 声明display为友元函数
private:
    int hour;
    int minute;
    int sec;
...
void display(const Time& t, const Date& d) // display的作用是输出年、月、日和时、分、秒
{
    // 引用Date类对象中的私有数据
    cout << d.month << "/" << d.day << "/" << d.year << endl;
    // 引用Time类对象中的私有数据
...
int main()
{
    Time t1(10, 13, 56);     // 定义Time类对象t1
    Date d1(12, 25, 2004);   // 定义Date类对象d1
    display(t1, d1);         // 调用全局函数display，实参是Time类对象t1和Date类对象d1
    return 0;
}