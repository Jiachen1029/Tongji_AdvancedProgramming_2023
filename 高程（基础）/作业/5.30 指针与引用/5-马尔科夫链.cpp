...
/****************** TODO ******************/
double(&next_status(double(&status)[STATUS_NUM], double(statusTransfer)[STATUS_NUM][STATUS_NUM]))[STATUS_NUM]
{
	double nextStatus[STATUS_NUM] = {};
	for (int i = 0; i < STATUS_NUM; i++)
	{
		for (int j = 0; j < STATUS_NUM; j++)
		{
			nextStatus[i] += status[j] * statusTransfer[j][i];
		}
	}
	for (int i = 0; i < STATUS_NUM; i++)
	{
