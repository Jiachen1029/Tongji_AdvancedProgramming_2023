...
void sort_info(StudentInfo* info_list, int info_num, char key)
{
    ...
    for (int i = 0; i < info_num - 1; ++i)
    {
        for (int j = 0; j < info_num - 1 - i; ++j)
        {
            ...
            if (key == 'y' && info_list[j].year < info_list[j + 1].year)
            {
                shouldSwap = true;
            }
            else if (key == 's' && info_list[j].score < info_list[j + 1].score)
            {
                shouldSwap = true;
            }
...
void print_info(const StudentInfo* info_list, int info_num)
{
    ...
    for (int i = 0; i < info_num; ++i)
    {
        ...
        cout << info_list[i].major<<" ";
        cout << info_list[i].year<<" ";
        cout << setprecision(2) << fixed << info_list[i].score << endl;
    }
    /**************** TODO-END ****************/
}
